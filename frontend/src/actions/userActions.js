...
// * ALL USERS LIST ACTION -------------------------------------------
export const listUsers = () => async (dispatch, getState) => {
  try {
    dispatch({ type: USER_LIST_REQUEST });

    // Get user logged in info from the state
    const {
      userLogin: { userInfo },
    } = getState();
    ...
    // get all users
    const { data } = await axios.get(`/api/users`, headerConfig);
...
export const deleteUser = (id) => async (dispatch, getState) => {
  try {
    ...
    // Delete a user by id
    await axios.delete(`/api/users/${id}`, headerConfig);
